...
#include <stdlib.h>
#include <math.h>
__global__ void ceil_kernel(const float* in, float* out, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) out[i] = ceilf(in[i]);
}
int main(int argc, char** argv) {
    ...
    int t=256,b=(n+t-1)/t;
    for(int i=0;i<warmup;i++) ceil_kernel<<<b,t>>>(d_in,d_out,n);
    cudaDeviceSynchronize();
    cudaEvent_t s,e; cudaEventCreate(&s); cudaEventCreate(&e);
    cudaEventRecord(s);
    for(int i=0;i<iters;i++) ceil_kernel<<<b,t>>>(d_in,d_out,n);
