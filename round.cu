...
#include <stdlib.h>
#include <math.h>
__global__ void round_kernel(const float* in, float* out, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) out[i] = roundf(in[i]);
}
int main(int argc, char** argv) {
    int n = (argc > 1) ? atoi(argv[1]) : 8, warmup=10, iters=100;
    float *h=(float*)malloc(n*sizeof(float));
    for(int i=0;i<n;i++) h[i]=(float)(i%5+1)*0.5f;
    float *d_in,*d_out; cudaMalloc(&d_in,n*4); cudaMalloc(&d_out,n*4);
    cudaMemcpy(d_in,h,n*4,cudaMemcpyHostToDevice);
    int t=256,b=(n+t-1)/t;
    for(int i=0;i<warmup;i++) round_kernel<<<b,t>>>(d_in,d_out,n);
    cudaDeviceSynchronize();
    cudaEvent_t s,e; cudaEventCreate(&s); cudaEventCreate(&e);
    cudaEventRecord(s);
    for(int i=0;i<iters;i++) round_kernel<<<b,t>>>(d_in,d_out,n);
