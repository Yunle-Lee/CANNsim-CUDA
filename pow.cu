...
#include <stdlib.h>
#include <math.h>
__global__ void kern(const float* a, const float* b, float* out, int n) {
...
int main(int argc, char** argv) {
    ...
    for(int i=0;i<n;i++){ha[i]=(float)(i%7+1)*0.5f; hb[i]=(float)(i%5+1)*0.3f+0.1f;}
    ...
    int t=256,b=(n+t-1)/t;
    for(int i=0;i<warmup;i++) kern<<<b,t>>>(da,db,dc,n);
    cudaDeviceSynchronize();
    cudaEvent_t s,e; cudaEventCreate(&s); cudaEventCreate(&e);
    cudaEventRecord(s);
    for(int i=0;i<iters;i++) kern<<<b,t>>>(da,db,dc,n);
